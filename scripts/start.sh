...
start_vpn_tunnel() {
    ...
    local EXISTING_CONTAINER=$(docker ps -q -f name=passage-${VPN_CONFIG_NUM})
    if [ ! -z "$EXISTING_CONTAINER" ]; then
        
        local CURRENT_PORT=$(docker port passage-${VPN_CONFIG_NUM} 1080/tcp | cut -d ':' -f 2)
        echo "SOCKS5 proxy available at: localhost:${CURRENT_PORT}"
        return 0
    fi
    
    
    local USED_PORTS=$(docker ps --filter "name=passage-" --format "{{.Ports}}" | grep -oP "0.0.0.0:\K\d+" || echo "")
    ...
    local COMPOSE_PROJECT_NAME="nordhell-${VPN_CONFIG_NUM}"
    export COMPOSE_PROJECT_NAME
    
    export VPN_CONFIG_NUM
    
    if [ "$FORCE_BUILD" = "true" ]; then
        echo "Building tunnel [$VPN_CONFIG_NUM] with --no-cache"
    ...
    if ! docker ps -q --filter "name=passage-${VPN_CONFIG_NUM}" | grep -q .; then
    ...
    echo "-------------------------------------"
    echo "LLUSTR TNNL:"
    echo "  Container    : passage-${VPN_CONFIG_NUM}"
